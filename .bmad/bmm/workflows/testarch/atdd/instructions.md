...
## ⚠️ Critical: Prevent Recursive Test Execution

**NEVER put these in test files:**
- `execSync('npm test')` - Causes infinite recursion
- `spawn('npm test')` - Causes process loops
- Testing the test framework itself

**Instead use static validation:**
```typescript
// ✅ OK: Check files and content
...
6. **Verify Tests Fail Initially**
   ...
   **Critical verification:**
   ...
   - **⚠️ SAFETY CHECK**: If tests hang, check for `execSync('npm test')` calls
   - Failure should be due to missing implementation, not infinite recursion
...
- [ ] **⚠️ No recursive `execSync('npm test')` calls in any test file**
